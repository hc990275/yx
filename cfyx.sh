...
main() {
    ...
    echo -e "${GREEN}=================================================="
    echo -e "      TG@sddzn 节点优选生成器 (50IP 版)"
    echo -e "==================================================${NC}"
    ...
    if [ ${#valid_urls[@]} -gt 0 ]; then
        if [ ${#valid_urls[@]} -eq 1 ]; then
            ...
            echo -e "${YELLOW}自动选择模板: ${valid_ps_names[0]}${NC}"
        else
        ...
        echo -e "${YELLOW}手动输入 vmess:// 链接:${NC}"
        read selected_url
    fi
    ...
    echo "---"
    echo -e "${GREEN}完成! 所有节点别名已设定为 TG@sddzn-IP。${NC}"
}
